...
use App\Models\vwObjectChemFieldsUsage;
...
class ChemicalController extends Controller
{
    ...
    public function totalUsage(Request $r) {
        ...
        $data = vwObjectChemFieldsUsage::selectRaw("top 5000 *")
            ->whereRaw($where)
    ...
    public function totalUsageChart(Request $r) {
        ...
        $query = "select cast([StartDateTime] as date) as [StartDateTime] ,ChemicalName,sum(usage) as usage from vwObjectChemFieldsUsage where {$where} group by cast([StartDateTime] as date),ChemicalName order by [StartDateTime]";

        //$data['data'] = DB::select($query);
        $data['data'] = vwObjectChemFieldsUsage::selectRaw("cast([StartDateTime] as date) as [StartDateTime] ,ChemicalName,sum(usage) as usage")
        ->whereRaw($where)
        ->groupByRaw('cast([StartDateTime] as date),ChemicalName')
        ->orderByRaw("[StartDateTime]")
        ->get();
    ...
    function BuildWhere(Request $r) {
        ...
        if($r['chemical']) {
            if(strlen($r->chemical)>1) $where .= " and [ChemicalName]='{$r->chemical}' ";
        }
